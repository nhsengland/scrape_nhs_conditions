...
class BaseItem(scrapy.Item):
    ...
    modified_at = (
        scrapy.Field()
    )  # Optional field. Should not be set in case of multiple documents on the same page.
...
class NHSConditionsAPISpider(scrapy.Spider):
    ...
    def start_requests(self):
        return [scrapy.Request(url=self.url, headers=self.headers, callback=self.parse)]
    ...
    def parse(self, response):
        ...
        if next_page_url is not None:
            return [scrapy.Request(next_page_url, self.parse, headers=self.headers)]
        return []
    ...
    def parse_item(self, response):
        ...
        item = HTMLItem(
            html=html, source_url=response.url, retrieved_at=datetime.datetime.now()
        )
...
def extract_text_from_html_json_chunk(
    ent: dict, text: str = None, key: str = "mainEntityOfPage"
) -> io.StringIO:
    ...
    for elt in ent:
        ...
        if key == "mainEntityOfPage":
            ...
            text.write(
                headline_field + " " + description_field + " " + text_field + " "
            )
...
def process_nhs_conditions_json(json_string: str) -> str:
    ...
    try:
        ...
        main_entity_text = extract_text_from_html_json_chunk(
            content.get("mainEntityOfPage", []), key="mainEntityOfPage"
        ).getvalue()
        has_part_text = extract_text_from_html_json_chunk(
            content.get("hasPart", []), key="hasPart"
        ).getvalue()
...
def unpack_json(json_doc="nhsconditions.jsonl", output_dir="docs/"):
    ...
    with jsonlines.open(json_doc) as reader:
        for index, obj in enumerate(reader):
            ...
            if extracted_text != "":
                with open(
                    f"{output_dir}{page_name}.txt", "w", encoding="utf-8"
                ) as file:
...
def run_nhs_conditions_scraper(output_file="nhs_conditions_raw.jsonl"):
    """Runs the NHS Conditions API scraper, outputting the results to a json lines file"""
    ...
    process.crawl(NHSConditionsAPISpider)
